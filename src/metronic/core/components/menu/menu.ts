...
import { MenuConfigInterface, MenuInterface } from './types';
import { MenuItemToggleType, MenuItemTriggerType } from './types';
import { OptionType } from '../../types';
import { _ACCESSIBILITY_KEYS } from '../constants';
import { Dropdown } from '../dropdown';
declare global {
	interface Window {
		_MENU_INITIALIZED: boolean;
	}
}

export class Menu extends Component implements MenuInterface {
	protected override _name: string = 'menu';
	protected override _defaultConfig: MenuConfigInterface = {
		dropdownZindex: '105',
		dropdownHoverTimeout: 200,
		accordionExpandAll: false
	};
	protected override _config: MenuConfigInterface = this._defaultConfig;
	protected _disabled: boolean = false;

	constructor(element: HTMLElement, config?: MenuConfigInterface) {
		super();
	...
	protected _mouseover(element: HTMLElement): void {
		...
		if (Data.get(itemElement, 'hover') === '1') {
			clearTimeout(Data.get(itemElement, 'timeout') as number);
			Data.remove(itemElement, 'hover');
			Data.remove(itemElement, 'timeout');
		}
	...
	protected _mouseout(element: HTMLElement): void {
		...
		Data.set(itemElement, 'hover', '1');
		Data.set(itemElement, 'timeout', timeout);
	}
	...
	protected _show(itemElement: HTMLElement): void {
		...
		Data.set(itemElement, 'toggle', this._getItemToggleMode(itemElement));
	}
	...
	protected _reset(itemElement: HTMLElement): void {
		...
		if (
			Data.has(itemElement, 'toggle') &&
			Data.get(itemElement, 'toggle') !== this._getItemToggleMode(itemElement)
		) {
	...
	protected _isItemParentShown(itemElement: HTMLElement): boolean {
		const parents = Dom.parents(itemElement, '.menu-item.show');
	...
	protected _getItemLinkElement(itemElement: HTMLElement): HTMLElement {
		return Dom.child(itemElement, '.menu-link, .menu-toggle');
	}
	...
	protected _getItemSubElement(itemElement: HTMLElement): HTMLElement {
		if (itemElement.classList.contains('menu-dropdown') === true || itemElement.classList.contains('menu-accordion') === true) {
			return itemElement;
		} else if (Data.has(itemElement, 'sub')) {
			return Data.get(itemElement, 'sub') as HTMLElement;
		} else {
			return Dom.child(itemElement, '.menu-dropdown, .menu-accordion');
		}
	}

	protected _getItemToggleMode(itemElement: HTMLElement): MenuItemToggleType {
	...
	protected _getItemElement(element: HTMLElement): HTMLElement {
		...
		if (Data.has(element, 'item')) {
			return Data.get(element, 'item') as HTMLElement;
		}
		...
		if (subElement) {
			if (Data.has(subElement, 'item') === true) {
				return Data.get(subElement, 'item') as HTMLElement;
			}
		}
	...
	protected _getItemParentElement(itemElement: HTMLElement): HTMLElement {
		...
		if (subElement && Data.has(subElement, 'item')) {
			return Data.get(subElement, 'item') as HTMLElement;
		}
	...
	protected _getItemChildElement(itemElement: HTMLElement): HTMLElement {
		...
		if (Data.has(itemElement, 'sub')) {
			selector = Data.get(itemElement, 'sub') as HTMLElement;
		}
	...
	protected _showDropdown(itemElement: HTMLElement): void {
		...
		if (parseInt(Dom.getCssProp(subElement, 'z-index')) > zindex) {
      zindex = parseInt(Dom.getCssProp(subElement, 'z-index'));
    }
		if (Dom.getHighestZindex(itemElement) > zindex) {
			zindex = Dom.getHighestZindex(itemElement) + 1;
		}
		...
		if (this._getItemOption(itemElement, 'overflow') === true) {
			...
			Data.set(itemElement, 'sub', subElement);
			Data.set(subElement, 'item', itemElement);
			Data.set(subElement, 'menu', this);
		} else {
			Data.set(subElement, 'item', itemElement);
		}
	...
	protected _hideDropdown(itemElement: HTMLElement): void {
		...
		if (this._getItemOption(itemElement, 'overflow') === true) {
			...
			if (itemElement.classList.contains('menu-item')) {
				...
				if (!this._element) return;
				Dom.insertAfter(this._element, itemElement);
			}

			Data.remove(itemElement, 'sub');
			Data.remove(subElement, 'item');
			Data.remove(subElement, 'menu');
		}
	...
	protected _initDropdownPopper(itemElement: HTMLElement, subElement: HTMLElement): void {
		...
		if (reference) {
			...
			Data.set(itemElement, 'popper', popper);
		}
	}

	protected _destroyDropdownPopper(itemElement: HTMLElement): void {
		if (Data.has(itemElement, 'popper')) {
			(Data.get(itemElement, 'popper') as PopperInstance).destroy();
			Data.remove(itemElement, 'popper');
		}
	}
	...
	protected _showAccordion(itemElement: HTMLElement): void {
		...
		if (Data.has(itemElement, 'popper') === true) {
			this._hideDropdown(itemElement);
		}
	...
	protected _getItemOption(element: HTMLElement, name: string): OptionType {
	...
	// General Methods
	public getItemTriggerMode(itemElement: HTMLElement): MenuItemTriggerType {
		return this._getItemOption(itemElement, 'trigger') as MenuItemTriggerType;
	}

	public getItemToggleMode(element: HTMLElement): MenuItemToggleType {
		return this._getItemToggleMode(element) as MenuItemToggleType;
	}
	...
	// Statics methods
	public static getInstance(element: HTMLElement): Menu {
		if (!element) {
			return null;
		}

		// Element has menu DOM reference in it's DATA storage
		if (Data.has(element, 'menu')) {
			return Data.get(element, 'menu') as Menu;
		}

		// Element has .menu parent
		const menuElement = element.closest('.menu') as HTMLElement;
		if (menuElement && Data.has(menuElement, 'menu')) {
			return Data.get(menuElement, 'menu') as Menu;
		...
		if (subElement && Data.has(subElement, 'menu')) {
			return Data.get(subElement, 'menu') as Menu;
		}
	...
	public static getOrCreateInstance(element: HTMLElement, config?: MenuConfigInterface): Menu {
		return this.getInstance(element) || new Menu(element, config);
	}
	...
	public static hide(skipElement?: HTMLElement): void {
		...
		itemElements.forEach((itemElement) => {
			const menu = Menu.getInstance(itemElement as HTMLElement);
	...
	public static updateDropdowns(): void {
		...
		itemElements.forEach((itemElement) => {
			if (Data.has(itemElement as HTMLElement, 'popper')) {
				(Data.get(itemElement as HTMLElement, 'popper') as PopperInstance).forceUpdate();
			}
		});
	}
	...
	public static updateByLinkAttribute(value: string, name: string = 'href'): void {
		...
		elements.forEach((element) => {
			const menu = Menu.getInstance(element as HTMLElement);
	...
	public static findFocused() {
		...
		if (linkElement && Dom.isVisible(linkElement)) {
			return linkElement;
		} else {
			return null;
		}
	...
	public static getFocusLink(linkElement: HTMLElement, direction: 'next' | 'previouse', preFocus = false): HTMLElement {
		...
		if (direction === 'next') {
			...
			if (nextElement && (nextElement.matches('.menu-accordion' + (!preFocus ? '.show' : '')) || nextElement.matches('.menu-dropdown' + (!preFocus ? '.show' : '')))) {
				const itemElement2 = Dom.child(nextElement, '.menu-item') as HTMLElement;
				return Dom.child(itemElement2, '.menu-link') as HTMLElement;
			} else {
				...
				if (nextElement2 && nextElement2.classList.contains('menu-item')) {
					const nextLink = Dom.child(nextElement2, '.menu-link') as HTMLElement;
					if (nextLink) {
						return nextLink;
					}
				}
			...
			if (prevElement) {
				if (prevElement && prevElement.classList.contains('menu-item')) {
					const nextLink = Dom.child(prevElement, '.menu-link') as HTMLElement;
					if (nextLink) {
						return nextLink;
					}
				}
	...
	public static handleKeyboard() {
		document.addEventListener('keydown', (event: KeyboardEvent) => {
			if (_ACCESSIBILITY_KEYS.includes(event.key)  && !(event.ctrlKey || event.altKey || event.shiftKey)) {
				...
				if (event.key === 'Escape') {
					...
					items.forEach((item) => {
						const menu = Menu.getInstance(item as HTMLElement);
	...
	public static handleMouseover() {
		EventHandler.on(
			...
			(event: Event, target: HTMLElement) => {
				const menu = Menu.getInstance(target);
		...
		window.addEventListener('resize', () => {
			...
			Utils.throttle(
				...
				() => {
					...
					elements.forEach((element) => {
						Menu.getInstance(element as HTMLElement)?.update();
					});
				},
				200
			);
	...
	public static createInstances(): void {
		...
		elements.forEach((element) => {
			new Menu(element as HTMLElement);
		});
	}
	...
	public static init(): void {
		...
		if (window._MENU_INITIALIZED !== true) {
			Menu.initHandlers();
			window._MENU_INITIALIZED = true;
		}
	}
}