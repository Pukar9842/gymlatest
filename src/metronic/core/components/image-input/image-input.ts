...
import {ImageInputInterface, ImageInputConfigInterface} from './types';

export class ImageInput extends Component implements ImageInputInterface {
  protected override _name: string = 'image-input';
  protected override _defaultConfig: ImageInputConfigInterface = {
		hiddenClass: 'hidden'
	};
  ...
  constructor(element: HTMLElement, config: ImageInputConfigInterface = null) {
    super();
  ...
  protected _handlers(): void {
    EventHandler.on(this._element, '.image-input-placeholder', 'click', (event: Event) => {
  ...
  public static getInstance(element: HTMLElement): ImageInput {
    if (!element) return null;

		if (Data.has(element, 'image-input')) {
			return Data.get(element, 'image-input') as ImageInput;
		}

		if (element.getAttribute('data-image-input') === "true") {
			return new ImageInput(element);
		}

    return null;
  }

  public static getOrCreateInstance(element: HTMLElement, config?: ImageInputConfigInterface): ImageInput {
    return this.getInstance(element) || new ImageInput(element, config);
  }
  ...
  public static createInstances(): void {
    ...
    elements.forEach((element) => {
      new ImageInput(element as HTMLElement);
    });
  }
