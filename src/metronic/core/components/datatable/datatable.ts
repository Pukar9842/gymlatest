...
import { DataTableDataInterface, DataTableInterface, DataTableConfigInterface as DataTableConfigInterface, DataTableSortOrderInterface, DataTableStateInterface, DataTableColumnFilterInterface, DataTableAttributeInterface } from './types';
import EventHandler from '../../helpers/event-handler';
import { DatatableCheckChangePayloadInterface } from './types';
 ...
 * @classdesc A custom Component class that integrates server-side API, pagination, and sorting functionality into a table.
 ...
 * @param {DataTableConfigInterface} [config] Additional configuration options
 */
export class DataTable<T extends DataTableDataInterface> extends Component implements DataTableInterface {
	protected override _name: string = 'datatable';
	protected override _config: DataTableConfigInterface;
	protected override _defaultConfig: DataTableConfigInterface;
	...
	constructor(element: HTMLElement, config?: DataTableConfigInterface) {
		super();
	 ...
	 */
	private _initDefaultConfig(config?: DataTableConfigInterface): DataTableConfigInterface {
		return {
			/**
			...
			sort: {
				 ...
				 */
				callback: (data: T[], sortField: keyof T | number, sortOrder: DataTableSortOrderInterface): T[] => {
					/**
			 ...
			 * Private properties
			 */
			_state: {} as DataTableStateInterface,

			...config

		} as DataTableConfigInterface;
	}
	...
	private _localTableContentInvalidate(): boolean {
		const checksum: string = Utils.checksum(JSON.stringify(this._tbodyElement.innerHTML));
	...
	private _tableConfigInvalidate(): boolean {
		...
		const checksum: string = Utils.checksum(JSON.stringify(restConfig));
	 ...
	 *
	 * @returns {{originalData: T[], originalDataAttributes: DataTableAttributeInterface[]}} - An object containing an array of original data and an array of original data attributes.
	 */
	private _localExtractTableContent(): { originalData: T[]; originalDataAttributes: DataTableAttributeInterface[]; } {
		const originalData: T[] = [];
		const originalDataAttributes: DataTableAttributeInterface[] = [];

		const rows = this._tbodyElement.querySelectorAll<HTMLTableRowElement>('tr');

		rows.forEach((row: HTMLTableRowElement) => {
			const dataRow: T = {} as T;
			const dataRowAttribute: DataTableAttributeInterface = {} as DataTableAttributeInterface;
	...
	private _getQueryParamsForFetchRequest(): URLSearchParams {
		...
		if (Array.isArray(filters) && filters.length) {
			queryParams.set('filters', JSON.stringify(filters.map((filter: DataTableColumnFilterInterface) => ({
	 ...
	 */
	private _setSortIcon(sortField: keyof T, sortOrder: DataTableSortOrderInterface): void {
	 ...
	 */
	private _toggleSortOrder(sortField: keyof T | number): DataTableSortOrderInterface {
	...
	private _saveState(): void {
		...
		if (ns) {
			localStorage.setItem(ns, JSON.stringify(this.getState() as DataTableStateInterface));
		}
	}
	 ...
	 */
	private _loadState(): DataTableStateInterface | null {
		const stateString = localStorage.getItem(this._tableNamespace());
		if (!stateString) return null;

		try {
			const state = JSON.parse(stateString) as DataTableStateInterface;
	...
	protected _change(checked: boolean): void {
		const payload: DatatableCheckChangePayloadInterface = { cancel: false };
	 ...
	 * @returns {DataTableStateInterface} The current state of the table.
	 */
	public getState(): DataTableStateInterface {
		return {
			/**
	 ...
	 */
	public setFilter(filter: DataTableColumnFilterInterface): DataTable<T> {
	 ...
	 * Static variables
	 */
	private static _instances = new Map<HTMLElement, DataTable<DataTableDataInterface>>();

	/**
	 * Create DataTable instances for all elements with a data-datatable="true" attribute.
	 *
	 * This function should be called after the control(s) have been
	 * loaded and parsed by the browser. It will create instances of
	 * DataTable for all elements with a data-datatable="true" attribute.
	 */
	public static createInstances(): void {
		...
		elements.forEach((element) => {
			if (element.hasAttribute('data-datatable') && !element.classList.contains('datatable-initialized')) {
				/**
				 * Create an instance of DataTable for the given element
	...
	/**
	 * Get the DataTable instance for a given element.
	 *
	 * @param element The element to retrieve the instance for
	 * @returns The DataTable instance or undefined if not found
	 */
	public static getInstance(element: HTMLElement): DataTable<DataTableDataInterface> | undefined {
		return this._instances.get(element);
	}

	/**
	 * Initializes all DataTable instances on the page.
	 *
	 * This function should be called after the control(s) have been
	 * loaded and parsed by the browser.
	 */
	public static init(): void {
		// Create instances of DataTable for all elements with a
		// data-datatable="true" attribute
		DataTable.createInstances();
	}
