...
import { ModalInterface, ModalConfigInterface } from './types';

declare global {
  interface Window {
    _MODAL_INITIALIZED: boolean;
  }
}

export class Modal extends Component implements ModalInterface {
  protected override _name: string = 'modal';
  protected override _defaultConfig: ModalConfigInterface = {
    zindex: '90',
    backdrop: true,
  ...
  protected override _config: ModalConfigInterface = this._defaultConfig;
  ...
  constructor(element: HTMLElement, config?: ModalConfigInterface) {
    super();
  ...
  protected _createBackdrop(): void {
    if (!this._element) return;
    const zindex: number = parseInt(Dom.getCssProp(this._element, 'z-index'));
  ...
  public static getInstance(element: HTMLElement): Modal {
		if (!element) return null;

		if (Data.has(element, 'modal')) {
			return Data.get(element, 'modal') as Modal;
		}
  ...
  public static getOrCreateInstance(element: HTMLElement, config?: ModalConfigInterface): Modal {
    return this.getInstance(element) || new Modal(element, config);
  }
  ...
  public static hide(): void {
    ...
    elements.forEach((element) => {
      const modal = Modal.getInstance(element as HTMLElement);
  ...
  public static handleToggle(): void {
    EventHandler.on(document.body, '[data-modal-toggle]', 'click', (event: Event, target: HTMLElement) => {
  ...
  public static handleDismiss(): void {
    EventHandler.on(document.body, '[data-modal-dismiss]', 'click', (event: Event, target: HTMLElement) => {
      event.stopPropagation();
  ...
  public static createInstances(): void {
    ...
    elements.forEach((element) => {
      new Modal(element as HTMLElement);
    });
  }

  public static init(): void {
    Modal.createInstances();

    if (window._MODAL_INITIALIZED !== true) {
      Modal.handleToggle();
      Modal.handleDismiss();
      Modal.handleClickAway();
      Modal.handleKeyword();      
      window._MODAL_INITIALIZED = true;
    }
  }
}
