...
import { ScrollableInterface, ScrollableConfigInterface } from './types';

declare global {
  interface Window {
    _SCROLL_INITIALIZED: boolean;
  }
}

export class Scrollable extends Component implements ScrollableInterface {
  protected override _name: string = 'scrollable';
  protected override _defaultConfig: ScrollableConfigInterface = {
    save: true,
    dependencies: '',
    wrappers: '',
    offset: ''
  };
  protected override _config: ScrollableConfigInterface = this._defaultConfig;
  protected _elementId: string | null = null;

  constructor(element: HTMLElement, config?: ScrollableConfigInterface) {
    super();
  ...
  protected _getAutoHeight(): string {
    if (!this._element) return '';
    let height = Dom.getViewPort().height;
    ...
    if (dependencies && dependencies.length > 0) {
      ...
      elements.forEach((element) => {
        if (Dom.getCssProp(element as HTMLElement, 'display') === 'none') {
          return;
        }
  ...
  public static getInstance(element: HTMLElement): Scrollable {
    if (!element) return null;

		if (Data.has(element, 'scrollable')) {
			return Data.get(element, 'scrollable') as Scrollable;
		}

		if (element.getAttribute('data-scrollable') === "true") {
			return new Scrollable(element);
		}

    return null;
  }

  public static getOrCreateInstance(element: HTMLElement, config?: ScrollableConfigInterface): Scrollable {
    return this.getInstance(element) || new Scrollable(element, config);
  }
  ...
  public static createInstances(): void {
    ...
    elements.forEach((element) => {
      new Scrollable(element as HTMLElement);
    });
  }
  ...
  public static handleResize(): void {
    window.addEventListener('resize', () => {
      ...
      Utils.throttle(timer, function () {
        ...
        elements.forEach((element) => {
          Scrollable.getInstance(element as HTMLElement)?.update();
        });
      }, 200);
    });
  }
  ...
  public static init(): void {
    Scrollable.createInstances();

    if (window._SCROLL_INITIALIZED !== true) {
      Scrollable.handleResize();

      window._SCROLL_INITIALIZED = true;
    }
  }
}
