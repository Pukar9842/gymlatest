...
import { ScrollspyInterface, ScrollspyConfigInterface } from './types';

export class Scrollspy extends Component implements ScrollspyInterface {
  protected override _name: string = 'scrollspy';
  protected override _defaultConfig: ScrollspyConfigInterface = {
    target: 'body',
    offset: 0,
    smooth: true
  };
  protected override _config: ScrollspyConfigInterface = this._defaultConfig;
  protected _targetElement: HTMLElement | Document | null = null;
  protected _anchorElements: NodeListOf<HTMLElement> | null = null;

  constructor(element: HTMLElement, config?: ScrollspyConfigInterface) {
    super();
    ...
    const targetElement = this._getTarget() === 'body' ? document : Dom.getElement(this._getTarget());
  ...
  protected _handlers(): void {
    ...
    EventHandler.on(this._element, '[data-scrollspy-anchor]', 'click', (event: Event, target: HTMLElement) => {
  ...
  protected _scrollTo(anchorElement: HTMLElement): void {
    ...
    const sectionElement = Dom.getElement(anchorElement.getAttribute('href'));
  ...
  protected _updateAnchor(anchorElement: HTMLElement): void {
    ...
    if (!Dom.isVisible(anchorElement)) return;
    ...
    if ((scrollPosition + offset) >= offsetTop) {
      ...
      const parentAnchorElements = Dom.parents(anchorElement, '[data-scrollspy-group]');
  ...
  public static getInstance(element: HTMLElement): Scrollspy {
    if (!element) return null;

		if (Data.has(element, 'scrollspy')) {
			return Data.get(element, 'scrollspy') as Scrollspy;
		}
  ...
  public static getOrCreateInstance(element: HTMLElement, config?: ScrollspyConfigInterface): Scrollspy {
    return this.getInstance(element) || new Scrollspy(element, config);
  }
  ...
  public static createInstances(): void {
    ...
    elements.forEach((element) => {
      new Scrollspy(element as HTMLElement);
    });
  }

  public static init(): void {
    Scrollspy.createInstances();
  }
}