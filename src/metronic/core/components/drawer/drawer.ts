...
import { DrawerInterface, DrawerConfigInterface } from './types';

declare global {
  interface Window {
    _DRAWER_INITIALIZED: boolean;
  }
}

export class Drawer extends Component implements DrawerInterface {
  protected override _name: string = 'drawer';
  protected override _defaultConfig: DrawerConfigInterface = {
    zindex: '100',
    enable: true,
    class: '',
    hiddenClass: 'hidden',
  ...
  protected override _config: DrawerConfigInterface = this._defaultConfig;
  ...
  constructor(element: HTMLElement, config?: DrawerConfigInterface) {
    super();
  ...
  protected _handleClose(): void {
    ...
    EventHandler.on(this._element, '[data-drawer-hide]', 'click', () => {
      this._hide();
    });
  }
  ...
  protected _show(relatedTarget?: HTMLElement): void {
    ...
    Dom.reflow(this._element);
    this._element.classList.add('open');
    
    Dom.transitionEnd(this._element, () => {
  ...
  protected _update(): void {
    if ((this._getOption('class') as string).length > 0) {
      if (this.isEnabled()) {
        Dom.addClass(this._element, this._getOption('class') as string);
      } else {
        Dom.removeClass(this._element, this._getOption('class') as string);
      }
    }    
  }
  ...
  protected _createBackdrop(): void {
    ...
    Dom.reflow(this._backdropElement);
    Dom.addClass(this._backdropElement, this._getOption('backdropClass') as string);
  ...
  protected _deleteBackdrop(): void {
    ...
    Dom.transitionEnd(this._backdropElement, () => {
      if (!this._backdropElement) return;
      Dom.remove(this._backdropElement);
    });
  }
  ...
  public static getInstance(element: HTMLElement): Drawer {
    if (!element) return null;

		if (Data.has(element, 'drawer')) {
			return Data.get(element, 'drawer') as Drawer;
		}
  ...
  public static getOrCreateInstance(element: HTMLElement, config?: DrawerConfigInterface): Drawer {
    return this.getInstance(element) || new Drawer(element, config);
  }
  ...
  public static hide(): void {
    ...
    elements.forEach((element) => {
      const drawer = Drawer.getInstance(element as HTMLElement);
  ...
  public static handleToggle(): void {
    EventHandler.on(document.body, '[data-drawer-toggle]', 'click', (event: Event, target: HTMLElement) => {
      ...
      const drawer = Drawer.getInstance(drawerElement as HTMLElement);
      if (drawer) {
        drawer.toggle();
      }
    });
  ...
  public static handleDismiss(): void {
    EventHandler.on(document.body, '[data-drawer-dismiss]', 'click', (event: Event, target: HTMLElement) => {
      event.stopPropagation();

      const modalElement = target.closest('[data-drawer="true"]') as HTMLElement;
      if (modalElement) {
        const modal = Drawer.getInstance(modalElement);
        if (modal) {
          modal.hide();
        }
      }      
  ...
  public static createInstances(): void {
    ...
    elements.forEach((element) => {
      new Drawer(element as HTMLElement);
    });
  }
  ...
  public static init(): void {
    ...
    if (window._DRAWER_INITIALIZED !== true) {
      Drawer.handleToggle();
      Drawer.handleDismiss();
      Drawer.handleResize();
      Drawer.handleClickAway();
      Drawer.handleKeyword();
      window._DRAWER_INITIALIZED = true;
    }
  }
}
