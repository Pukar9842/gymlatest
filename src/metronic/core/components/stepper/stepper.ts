...
import { StepperInterface, StepperConfigInterface } from './types';

export class Stepper extends Component implements StepperInterface {
  protected override _name: string = 'stepper';
  protected override _defaultConfig: StepperConfigInterface = {
    hiddenClass: 'hidden',
    activeStep: 1
  };
  protected override _config: StepperConfigInterface = this._defaultConfig;
  protected _activeStep: number = 0;
  protected _nextElement: HTMLElement | null = null;
  protected _backElement: HTMLElement | null = null;

  constructor(element: HTMLElement, config: StepperConfigInterface | null = null) {
    super();
  ...
  protected _update(): void {
    ...
    this._getItemElements().forEach((element, index) => {
      const contentElement = Dom.getElement(element.getAttribute('data-stepper-item'));
  ...
  protected _getItemElements(): Array<HTMLElement> {
    ...
    this._element.querySelectorAll('[data-stepper-item]').forEach((element) => {
      if (Dom.isVisible(element as HTMLElement)) {
  ...
  public static getInstance(element: HTMLElement): Stepper {
    if (!element) return null;

		if (Data.has(element, 'stepper')) {
			return Data.get(element, 'stepper') as Stepper;
		}

		if (element.getAttribute('data-stepper') === "true") {
			return new Stepper(element);
		}

    return null;
  }
  
  public static getOrCreateInstance(element: HTMLElement, config?: StepperConfigInterface): Stepper | null {
    return this.getInstance(element) || new Stepper(element, config);
  }
  ...
  public static createInstances(): void {
    ...
    elements.forEach((element) => {
      new Stepper(element as HTMLElement);
    });
  }

  public static init(): void {
    Stepper.createInstances();
  }
}
