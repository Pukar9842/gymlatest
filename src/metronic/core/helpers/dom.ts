...
import { OffsetType, OptionType, ViewPortType } from '../types';
...
const Dom = {
  ...
  getElement(element: HTMLElement | string): HTMLElement {
    ...
    if (element && (element as string).length > 0) {
      return document.querySelector(Utils.parseSelector((element as string)));
    }
  ...
  offsetOld(element: HTMLElement): OffsetType {
  ...
  offset(element: HTMLElement): OffsetType {
    if (!element) return { top: 0, left: 0, right: 0, bottom: 0 };
  ...
  child(element: HTMLElement, selector: string): HTMLElement {
    const children = Dom.children(element, selector);
  ...
  getDataAttributes(element: HTMLElement, prefix: string): object {
    ...
    prefix = Utils.camelCase(prefix);
    
    const attributes: { [key: string]: OptionType } = {};
    const keys = Object.keys(element.dataset).filter((key) => key.startsWith(prefix));

    for (const key of keys) {
      let normalizedKey = key.replace(prefix, '');
      normalizedKey = Utils.uncapitalize(normalizedKey);
      attributes[normalizedKey] = Utils.parseDataAttribute(element.dataset[key]);
    }
