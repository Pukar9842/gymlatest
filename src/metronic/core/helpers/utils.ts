...
const Utils = {
	...
	getBreakpoint(breakpoint: BreakpointType): number {
		const value: string = Utils.getCssVar(`--tw-${breakpoint}`);
	...
	parseDataAttribute(value: string): OptionType {
		if (value === 'true') {
			return true;
		}
		...
		try {
			return Utils.parseJson(value) as object;
		} catch {
			return value;
		}
	},
