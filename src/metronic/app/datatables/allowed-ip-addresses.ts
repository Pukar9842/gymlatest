...
interface IPAddressData extends DataTableDataInterface {
	id: number;
	status: string;
	ipAddress: string;
	lastSession: string;
...
const dataTableOptions: DataTableConfigInterface = {
	// Initialization options for the DataTable
	apiEndpoint: apiUrl,

	// mapResponse: (responseData): DatatableResponseDataInterface => {
	...
	columns: {
		select: {
			render: (
				item: keyof DataTableDataInterface,
				data: IPAddressData,
				context: DataTableInterface
			): string => {
...
const dataTable = new DataTable<IPAddressData>(element, dataTableOptions);
